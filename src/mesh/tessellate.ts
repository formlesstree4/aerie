...
import { Primitive, PrimType, type RawModel, type BLAS } from "../scene/scene";
...
/**
 * An indexed triangle mesh in LOCAL space. The same data feeds both the
 * ray-tracer BLAS (de-indexed soup) and the raster preview (BufferGeometry).
 */
interface PrimMesh {
  pos: number[]; // 3/vertex
  nor: number[]; // 3/vertex
  idx: number[]; // 3/triangle
}

class Builder implements PrimMesh {
  pos: number[] = [];
  nor: number[] = [];
  idx: number[] = [];

  vert(p: V3, n: V3): number {
  ...
  grid(uSeg: number, vSeg: number, fn: (i: number, j: number) => { p: V3; n: V3 }): void {
  ...
  fan(center: V3, n: V3, ring: V3[], flip: boolean): void {
    const c = this.vert(center, n);
    const start = this.pos.length / 3;
    for (const p of ring) this.vert(p, n);
    for (let i = 0; i < ring.length; i++) {
      const a = start + i;
      const b = start + ((i + 1) % ring.length);
      if (flip) this.tri(c, b, a);
      else this.tri(c, a, b);
    }
  }
}
...
function sphereMesh(r: number): PrimMesh {
  const b = new Builder();
  b.grid(SEG, RINGS, (i, j) => {
    const phi = (i / SEG) * Math.PI * 2;
    const theta = (j / RINGS) * Math.PI;
    const st = Math.sin(theta);
    const n: V3 = [st * Math.cos(phi), Math.cos(theta), st * Math.sin(phi)];
    return { p: [n[0] * r, n[1] * r, n[2] * r], n };
  });
  return b;
}

function boxMesh(a: number, bb: number, c: number): PrimMesh {
  const b = new Builder();
...
function torusMesh(R: number, r: number): PrimMesh {
  const b = new Builder();
  b.grid(SEG, RINGS, (i, j) => {
    const phi = (i / SEG) * Math.PI * 2;
    const theta = (j / RINGS) * Math.PI * 2;
    const ct = Math.cos(theta);
    const n: V3 = [ct * Math.cos(phi), Math.sin(theta), ct * Math.sin(phi)];
    const p: V3 = [(R + r * ct) * Math.cos(phi), r * Math.sin(theta), (R + r * ct) * Math.sin(phi)];
    return { p, n };
  });
  return b;
}

function cylinderMesh(r: number, h: number): PrimMesh {
  const b = new Builder();
  b.grid(SEG, 1, (i, j) => {
    const phi = (i / SEG) * Math.PI * 2;
...
function coneMesh(r1: number, h: number): PrimMesh {
...
function capsuleMesh(r: number, h: number): PrimMesh {
  const b = new Builder();
  // Cylindrical shaft.
  b.grid(SEG, 1, (i, j) => {
    const phi = (i / SEG) * Math.PI * 2;
...
function pyramidMesh(a: number, h: number): PrimMesh {
  const b = new Builder();
  const apex: V3 = [0, h, 0];
  const base: V3[] = [[-a, -h, -a], [a, -h, -a], [a, -h, a], [-a, -h, a]];
  for (let i = 0; i < 4; i++) {
    const p1 = base[i], p2 = base[(i + 1) % 4];
    const n = norm(
      (p1[1] - apex[1]) * (p2[2] - apex[2]) - (p1[2] - apex[2]) * (p2[1] - apex[1]),
      (p1[2] - apex[2]) * (p2[0] - apex[0]) - (p1[0] - apex[0]) * (p2[2] - apex[2]),
      (p1[0] - apex[0]) * (p2[1] - apex[1]) - (p1[1] - apex[1]) * (p2[0] - apex[0]),
    );
...
function octahedronMesh(s: number): PrimMesh {
  const b = new Builder();
...
export function primitiveLocalGeometry(prim: Primitive): BufferGeometry {
...
export function primitiveGeometry(prim: Primitive): BufferGeometry {
  const m = meshFor(prim);
  const g = new BufferGeometry();
  g.setAttribute("position", new Float32BufferAttribute(m.pos, 3));
  g.setAttribute("normal", new Float32BufferAttribute(m.nor, 3));
  ...
  const mat = new Matrix4().compose(
    prim.position,
    new Quaternion().setFromEuler(prim.rotation),
    new Vector3(1, 1, 1),
  );
  g.applyMatrix4(mat);
  return g;
...
function meshFor(prim: Primitive): PrimMesh {
  switch (prim.type) {
...
function buildBlas(m: PrimMesh): BLAS {
 ...
 */
export function convertPrimitive(prim: Primitive): { model: RawModel; group: Group } {
