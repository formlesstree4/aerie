import { Mesh, SkinnedMesh, Vector3, Matrix3, Matrix4, Box3, Group, } from "three";
...
function extractWorldTriangles(root, resolveMat) {
    const tris = [];
    const v = new Vector3();
    const n = new Vector3();
    root.traverse((obj) => {
        ...
        const skinned = mesh instanceof SkinnedMesh ? mesh : null;
        ...
        const uvAt = (i) => (uv ? [uv.getX(i), uv.getY(i)] : [0, 0]);
        ...
        for (const g of groups) {
            ...
            for (let f = g.start; f < end; f += 3) {
                ...
                tris.push(p0[0], p0[1], p0[2], 0, p1[0], p1[1], p1[2], 0, p2[0], p2[1], p2[2], 0, n0[0], n0[1], n0[2], 0, n1[0], n1[1], n1[2], cAt(a), // n1.w = vertex-color 0 (packed)
                n2[0], n2[1], n2[2], cAt(b), // n2.w = vertex-color 1 (packed)
                u0[0], u0[1], u1[0], u1[1], u2[0], u2[1], mid, cAt(c));
            }
        }
    });
    return tris;
...
function resolveMatInto(mat, materials, materialNames, texLayers, normalLayers) {
    ...
    if (std.normalMap?.image) {
        ...
        if (layer) {
            normalLayer = normalLayers.length;
            normalLayers.push(layer);
        }
    }
    materials.push(base.r, base.g, base.b, std.opacity ?? 1, emis.r * eInt, emis.g * eInt, emis.b * eInt, 0, metalness, roughness, texLayer, normalLayer);
}
/** Format-agnostic: bake a three.js scene graph into a placeable model. */
export function buildImportResult(root, name, animations) {
    ...
    if (worldTris.length === 0)
        throw new Error("No triangle geometry found in file.");
...
export function worldMesh(root) {
    root.updateMatrixWorld(true);
    const map = new Map();
...
export function bakePose(inner, matBase) {
...
export function computeFaceTangents(raw) {
    for (let i = 0; i < raw.length; i += TRI_STRIDE) {
