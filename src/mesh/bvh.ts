...
export interface TLASResult {
  nodes: Float32Array<ArrayBuffer>;
  order: Uint32Array<ArrayBuffer>; // instance indices in leaf order
  nodeCount: number;
}
...
export function buildTLAS(mins: Float32Array, maxs: Float32Array, count: number): TLASResult {
  if (count === 0) {
  ...
  for (let i = 0; i < count; i++) {
    idx[i] = i;
    for (let a = 0; a < 3; a++) cent[i * 3 + a] = (mins[i * 3 + a] + maxs[i * 3 + a]) * 0.5;
  }
  ...
  function bounds(start: number, end: number) {
    ...
    for (let i = start; i < end; i++) {
      const t = idx[i];
      lx = Math.min(lx, mins[t * 3]); ly = Math.min(ly, mins[t * 3 + 1]); lz = Math.min(lz, mins[t * 3 + 2]);
      hx = Math.max(hx, maxs[t * 3]); hy = Math.max(hy, maxs[t * 3 + 1]); hz = Math.max(hz, maxs[t * 3 + 2]);
    }
    return { lx, ly, lz, hx, hy, hz };
  }
  ...
  for (let ni = nodeCount - 1; ni >= 0; ni--) {
    ...
    if (b > 0) {
      ...
      for (let k = 0; k < b; k++) {
        ...
        lx = Math.min(lx, mins[ii * 3]); ly = Math.min(ly, mins[ii * 3 + 1]); lz = Math.min(lz, mins[ii * 3 + 2]);
        hx = Math.max(hx, maxs[ii * 3]); hy = Math.max(hy, maxs[ii * 3 + 1]); hz = Math.max(hz, maxs[ii * 3 + 2]);
      }
    } else {
      const lo = nodes[o + 3] * 8;         // left child node
      const ro = (-b - 1) * 8;             // right child node
      lx = Math.min(nodes[lo], nodes[ro]); ly = Math.min(nodes[lo + 1], nodes[ro + 1]); lz = Math.min(nodes[lo + 2], nodes[ro + 2]);
      hx = Math.max(nodes[lo + 4], nodes[ro + 4]); hy = Math.max(nodes[lo + 5], nodes[ro + 5]); hz = Math.max(nodes[lo + 6], nodes[ro + 6]);
    }
    nodes[o] = lx; nodes[o + 1] = ly; nodes[o + 2] = lz;
    nodes[o + 4] = hx; nodes[o + 5] = hy; nodes[o + 6] = hz;
