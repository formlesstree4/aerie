import { BufferGeometry, Float32BufferAttribute, Mesh, MeshStandardMaterial, Group, Vector3, DataTexture, RGBAFormat, SRGBColorSpace, } from "three";
...
export function trisToGeometry(tris) {
    ...
    const order = [...Array(tcount).keys()].sort((a, b) => tris[a * TRI_STRIDE + 30] - tris[b * TRI_STRIDE + 30]);
    ...
    for (let ti = 0; ti < tcount; ti++) {
        ...
        for (let v = 0; v < 3; v++) {
            ...
            pos[d3 + 1] = tris[vo + 1];
            pos[d3 + 2] = tris[vo + 2];
            nor[d3] = tris[o + 12 + v * 4];
            nor[d3 + 1] = tris[o + 12 + v * 4 + 1];
            nor[d3 + 2] = tris[o + 12 + v * 4 + 2];
            const uo = o + 24 + v * 2;
            uv[d2] = tris[uo];
            uv[d2 + 1] = tris[uo + 1];
        }
    ...
    for (const grp of groups)
        g.addGroup(grp.start, grp.count, grp.mat);
    return g;
}
...
function threeMatFor(desc) {
    ...
    if (desc.tex)
        m.map = texFromBytes(desc.tex, true);
    if (desc.normal)
        m.normalMap = texFromBytes(desc.normal, false);
...
function brushFor(input) {
    ensureUV(input.geometry);
    if (!input.geometry.getAttribute("normal"))
        input.geometry.computeVertexNormals();
...
export function csgBuildModel(inputs, name) {
    ...
    for (let i = 1; i < solids.length; i++)
        acc = evaluator.evaluate(acc, brushFor(solids[i]), ADDITION);
    for (const neg of negatives)
        acc = evaluator.evaluate(acc, brushFor(neg), SUBTRACTION);
    const geom = acc.geometry;
    const mats = Array.isArray(acc.material) ? acc.material : [acc.material];
...
function geometryToBlas(geom) {
    ...
    for (const g of geom.groups) {
        ...
        const cnt = g.count / 3;
        for (let t = startTri; t < startTri + cnt && t < triCount; t++)
            triMat[t] = g.materialIndex ?? 0;
    }
    ...
    for (let t = 0; t < triCount; t++) {
        ...
        for (let v = 0; v < 3; v++) {
            ...
            raw[o + v * 4] = pos.getX(i);
            raw[o + v * 4 + 1] = pos.getY(i);
            raw[o + v * 4 + 2] = pos.getZ(i);
            raw[o + 12 + v * 4] = nrm.getX(i);
            raw[o + 12 + v * 4 + 1] = nrm.getY(i);
            raw[o + 12 + v * 4 + 2] = nrm.getZ(i);
            const uo = o + 24 + v * 2;
            raw[uo] = uv ? uv.getX(i) : 0;
            raw[uo + 1] = uv ? uv.getY(i) : 0;
        }
        raw[o + 30] = triMat[t];
        raw[o + 19] = 0xffffff;
        raw[o + 23] = 0xffffff;
        raw[o + 31] = 0xffffff; // white vertex color
    }
