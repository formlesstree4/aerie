import type { BLAS } from "../scene/scene";
import { buildBVH } from "./bvh";
import { computeFaceTangents, TRI_STRIDE } from "./modelImport";

/**
 * An editable view of a BLAS: the de-indexed triangle soup is welded into unique
 * vertices (so moving a point moves all triangle corners that share it, avoiding
 * cracks). UVs and material id are preserved from the source triangles.
 */
export interface EditMesh {
  verts: Float32Array; // 3 floats/vertex, LOCAL space (mutable)
  cornerVert: Uint32Array; // 3/triangle → unique vertex index
  triCount: number;
  src: Float32Array; // original blas.tris (uv + matId stay constant)
}

export function buildEditMesh(blas: BLAS): EditMesh {
  const tris = blas.tris;
  const triCount = blas.triCount;
  const verts: number[] = [];
  const map = new Map<string, number>();
  const cornerVert = new Uint32Array(triCount * 3);
  const q = (n: number) => Math.round(n * 1e4) / 1e4; // weld tolerance
...
export function rebuildBlas(edit: EditMesh): BLAS {
  ...
  for (let t = 0; t < triCount; t++) {
    ...
    for (const i of [i0, i1, i2]) {
      vn[3 * i] += nx; vn[3 * i + 1] += ny; vn[3 * i + 2] += nz;
    }
  }
  for (let i = 0; i < nVerts; i++) {
    const x = vn[3 * i], y = vn[3 * i + 1], z = vn[3 * i + 2];
    const len = Math.hypot(x, y, z) || 1;
    vn[3 * i] = x / len; vn[3 * i + 1] = y / len; vn[3 * i + 2] = z / len;
  }
  ...
  for (let t = 0; t < triCount; t++) {
    ...
    for (let v = 0; v < 3; v++) {
      ...
      const po = base + v * 4;
      out[po] = verts[3 * i]; out[po + 1] = verts[3 * i + 1]; out[po + 2] = verts[3 * i + 2];
      const no = base + 12 + v * 4;
      out[no] = vn[3 * i]; out[no + 1] = vn[3 * i + 1]; out[no + 2] = vn[3 * i + 2];
    }
    for (let k = 24; k < 32; k++) out[base + k] = src[base + k]; // uv01, uv2m (uv + matId + vcolor2)
