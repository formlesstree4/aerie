import { Primitive, MeshInstance, PrimType, LightType, PrimPattern, CloudType, PRIM_LABELS, PATTERN_LABELS, CLOUD_LABELS, MAX_CLOUD_LAYERS, MAX_BOOL_GROUPS, Emitter, } from "./scene/scene";
...
export function buildUI(scene, getSpawn, hooks) {
    ...
    function buildInspector() {
        ...
        if (selected instanceof Emitter) {
            const e = selected;
            const typeOpts = [EmitterType.Campfire, EmitterType.Explosion,
                EmitterType.Smoke, EmitterType.Sparks, EmitterType.Fireworks, EmitterType.Magic]
            ...
            box.append(group("Timing", (g) => {
                g.append(dropdown("mode", e.loop ? 1 : 0, [[1, "Loop (continuous)"], [0, "One-shot (burst)"]], (v) => { e.loop = v === 1; refresh(); }));
                if (!e.loop)
                    g.append(slider("burst @", e.burstTime, 0, 10, 0.05, (v) => (e.burstTime = v)));
            ...
            box.append(el("div", "hint", "Emitters flicker live in Preview. In Render they freeze at the timeline moment — scrub the cutscene or export a WebM to animate them."));
        }
    ...
    function buildList() {
        ...
        const items = [...scene.prims, ...scene.instances, ...scene.lights, ...scene.emitters];
    ...
    function buildMenubar() {
        ...
        const addMenu = menu("add", "Add", (pop) => {
            ...
            [EmitterType.Campfire, EmitterType.Explosion, EmitterType.Smoke,
                EmitterType.Sparks, EmitterType.Fireworks, EmitterType.Magic].forEach((t) => pop.append(menuItem(EMITTER_LABELS[t], () => { selection = [scene.addEmitter(t, getSpawn())]; hooks.onCommit(); })));
        });
