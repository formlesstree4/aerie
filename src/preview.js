import { WebGLRenderer, Scene as ThreeScene, PerspectiveCamera, DirectionalLight, HemisphereLight, Mesh, PlaneGeometry, MeshStandardMaterial, Color, Fog, AnimationMixer, Clock, MathUtils, DoubleSide, Float32BufferAttribute, Points, BufferGeometry, BufferAttribute, ShaderMaterial, AdditiveBlending, DynamicDrawUsage, } from "three";
...
export class Preview {
    ...
    particles = null;
    particleScratch = new Float32Array(MAX_PARTICLES * PARTICLE_FLOATS);
    particleTime = 0; // preview-local clock (loops so one-shots replay)
    ...
    ensureParticles() {
        ...
        const mk = (n) => new BufferAttribute(new Float32Array(MAX_PARTICLES * n), n).setUsage(DynamicDrawUsage);
    ...
    syncParticles(dt) {
        ...
        for (const e of this.app.emitters)
            win = Math.max(win, e.loop ? e.lifetime : e.burstTime + e.lifetime);
        ...
        const psize = g.getAttribute("psize");
        const pcol = g.getAttribute("pcolor");
        const posA = pos.array, sizeA = psize.array, colA = pcol.array;
        let n = 0;
        for (let i = 0; i < count; i++) {
            const o = i * PARTICLE_FLOATS;
            const op = buf[o + 7];
            if (op <= 0.003)
                continue; // dead/recycled slot
            posA[n * 3] = buf[o];
            posA[n * 3 + 1] = buf[o + 1];
            posA[n * 3 + 2] = buf[o + 2];
            sizeA[n] = buf[o + 3];
            colA[n * 3] = buf[o + 4] * op;
            colA[n * 3 + 1] = buf[o + 5] * op;
            colA[n * 3 + 2] = buf[o + 6] * op;
            n++;
        }
        g.setDrawRange(0, n);
        pos.needsUpdate = true;
        ...
        pts.material.uniforms.uScale.value = h / (2 * Math.tan(fov / 2));
    }
    render(cam) {
