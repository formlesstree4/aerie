...
import { TERRAIN_CATALOG, BASIS_ID, FRACTAL_ID } from "../gen/recipe";
import { EMITTER_LABELS, defaultEmitter } from "../gen/particles";
...
export var PrimType;
(function (PrimType) {
    PrimType[PrimType["Sphere"] = 0] = "Sphere";
    PrimType[PrimType["Box"] = 1] = "Box";
    PrimType[PrimType["Torus"] = 2] = "Torus";
    PrimType[PrimType["Cylinder"] = 3] = "Cylinder";
    PrimType[PrimType["Cone"] = 4] = "Cone";
    PrimType[PrimType["Octahedron"] = 5] = "Octahedron";
    PrimType[PrimType["Capsule"] = 6] = "Capsule";
    PrimType[PrimType["Pyramid"] = 7] = "Pyramid";
    PrimType[PrimType["RoundedBox"] = 8] = "RoundedBox";
})(PrimType || (PrimType = {}));
...
export var PrimPattern;
(function (PrimPattern) {
    PrimPattern[PrimPattern["Solid"] = 0] = "Solid";
    PrimPattern[PrimPattern["Checker"] = 1] = "Checker";
    PrimPattern[PrimPattern["Noise"] = 2] = "Noise";
    PrimPattern[PrimPattern["Wood"] = 3] = "Wood";
    PrimPattern[PrimPattern["Marble"] = 4] = "Marble";
    PrimPattern[PrimPattern["Gradient"] = 5] = "Gradient";
    PrimPattern[PrimPattern["Image"] = 6] = "Image";
...
export class Primitive {
    ...
    b = 5;
    c = 5;
    color = [0.8, 0.8, 0.85]; // primary / pattern color A
    reflectivity = 0;
    // Material
    ...
    constructor(type) {
        ...
        switch (type) {
            ...
            case PrimType.Capsule:
                this.a = 3;
                this.b = 4;
                break; // radius, half-height of the shaft
            case PrimType.Pyramid:
                this.a = 5;
                this.b = 5;
                break; // base half-width, half-height
...
export class Light {
    ...
    color = [1, 1, 1];
    intensity;
    softness; // angular/area softness for shadows
...
export class Emitter {
    ...
    constructor(type) {
        ...
        this.colorA = [...d.colorA];
        this.colorB = [...d.colorB];
        this.seed = d.seed;
        this.loop = d.loop;
        this.burstTime = d.burstTime;
    }
    clone() {
...
(function (CloudType) {
    ...
    CloudType[CloudType["Cirrocumulus"] = 1] = "Cirrocumulus";
    CloudType[CloudType["Cumulus"] = 2] = "Cumulus";
    CloudType[CloudType["Cumulonimbus"] = 3] = "Cumulonimbus";
    CloudType[CloudType["Stratus"] = 4] = "Stratus";
    CloudType[CloudType["Cirrostratus"] = 5] = "Cirrostratus";
    CloudType[CloudType["Altocumulus"] = 6] = "Altocumulus";
    CloudType[CloudType["Altostratus"] = 7] = "Altostratus";
    CloudType[CloudType["Nimbostratus"] = 8] = "Nimbostratus";
...
export function makeCloudLayer(type) {
    const seed = Math.random() * 100;
    const white = [1, 0.99, 0.97];
    switch (type) {
...
export class MeshInstance {
    ...
    quat = new Quaternion();
    scl = new Vector3();
    constructor(blasIndex, name) {
        this.blasIndex = blasIndex;
        this.name = name;
    }
    /** Write the world→local inverse model matrix (16 floats) at offset. */
    writeInvModel(out, offset) {
        this.quat.setFromEuler(this.rotation);
    ...
    clone() {
        ...
        m.group = this.group;
        if (this.landform)
            m.landform = structuredClone(this.landform);
        return m;
    }
}
export class Scene {
    ...
    blases = [];
    blasMatBase = []; // global material offset per BLAS (for re-baking)
    blasMatCount = []; // material count per BLAS (for PBR overrides)
    ...
    blasFile = [];
    instances = [];
    meshMaterials = []; // flat, MAT_FLOATS each, global
    meshMaterialNames = []; // one per material (global)
    meshTexLayers = [];
    meshNormalLayers = [];
    meshTexSize = 256;
    ...
    primImageSize = 256;
    primTexVersion = 0;
    primData = new Float32Array(MAX_PRIMS * PRIM_FLOATS);
    lightData = new Float32Array(MAX_LIGHTS * LIGHT_FLOATS);
    touch() {
        this.version++;
    }
    ...
    addCloudLayer(type) {
        if (this.world.cloudLayers.length >= MAX_CLOUD_LAYERS)
            return null;
    ...
    applyBiome(name) {
        ...
        switch (name) {
            case "alpine":
                w.terrainLow = [0.2, 0.32, 0.14];
                w.terrainRock = [0.34, 0.3, 0.27];
                w.terrainHigh = [0.92, 0.94, 0.97];
                w.waterEnabled = true;
                break;
            case "desert":
                w.terrainLow = [0.78, 0.66, 0.42];
                w.terrainRock = [0.62, 0.44, 0.28];
                w.terrainHigh = [0.88, 0.79, 0.58];
                w.waterEnabled = false;
                break;
            case "tundra":
                w.terrainLow = [0.55, 0.58, 0.5];
                w.terrainRock = [0.4, 0.4, 0.43];
                w.terrainHigh = [0.95, 0.96, 0.98];
                w.waterEnabled = true;
                break;
            case "badlands":
                w.terrainLow = [0.56, 0.31, 0.2];
                w.terrainRock = [0.4, 0.22, 0.16];
                w.terrainHigh = [0.72, 0.56, 0.4];
                w.waterEnabled = false;
                break;
        }
        this.touchWorld();
    ...
    applySkyPreset(name) {
        ...
        const set = (zen, hor, dir, col, intensity, exposure, layers, stars, haze) => {
            w.zenith = zen;
            w.horizon = hor;
            w.exposure = exposure;
            w.cloudLayers = layers;
            w.cloudsEnabled = layers.length > 0;
            w.starsEnabled = stars;
            w.hazeDensity = haze;
            if (sun) {
        ...
        const layer = (type, over) => ({ ...makeCloudLayer(type), ...over });
        switch (name) {
            case "day":
                set([0.16, 0.33, 0.72], [0.78, 0.84, 0.92], [0.4, 0.7, 0.3], [1, 0.96, 0.9], 3.2, 1.05, [layer(CloudType.Cumulus, { coverage: 0.4 })], false, 0.0016);
                break;
            case "sunset":
                set([0.2, 0.2, 0.42], [0.98, 0.46, 0.22], [0.75, 0.1, 0.2], [1.0, 0.5, 0.25], 2.4, 1.1, [layer(CloudType.Cirrus, { coverage: 0.55 }), layer(CloudType.Cumulus, { coverage: 0.3 })], false, 0.0035);
                break;
            case "night":
                set([0.01, 0.02, 0.06], [0.04, 0.06, 0.12], [0.3, 0.25, 0.2], [0.5, 0.62, 0.95], 0.3, 1.5, [], true, 0.001);
                break;
            case "overcast":
                set([0.52, 0.55, 0.6], [0.72, 0.74, 0.77], [0.3, 0.6, 0.2], [0.85, 0.85, 0.88], 1.2, 1.0, [layer(CloudType.Stratus, { coverage: 0.95 })], false, 0.004);
                break;
        }
    ...
    applyTimeOfDay(hours) {
        ...
        const dir = [ca * Math.sin(az), Math.sin(alt), ca * Math.cos(az)];
    ...
    materialDescriptor(blasIndex, mi) {
        const gi = (this.blasMatBase[blasIndex] ?? 0) + mi;
        const o = gi * MAT_FLOATS;
        const m = this.meshMaterials;
        const texIdx = m[o + 10];
    ...
    applyModelPBR(blasIndex, metalness, roughness) {
        const count = this.materialCount(blasIndex);
        for (let m = 0; m < count; m++)
            this.setMaterial(blasIndex, m, { metalness, roughness });
    }
    /** Swap in a freshly baked BLAS (e.g. a new animation pose). */
    replaceBlas(blasIndex, blas) {
    ...
    addModel(model, spawn) {
        ...
        for (let i = 0; i < tris.length; i += TRI_FLOATS)
            tris[i + TRI_MATID_OFFSET] += matBase;
        for (let m = 0; m < model.materials.length; m += MAT_FLOATS) {
            if (model.materials[m + 10] >= 0)
                model.materials[m + 10] += layerBase; // base-color layer
            if (model.materials[m + 11] >= 0)
                model.materials[m + 11] += normalBase; // normal layer
        }
        this.meshMaterials.push(...model.materials);
        for (const nm of model.materialNames)
            this.meshMaterialNames.push(nm);
        for (const layer of model.texLayers)
            this.meshTexLayers.push(layer);
        for (const layer of model.normalLayers)
            this.meshNormalLayers.push(layer);
    ...
    addLight(type, at) {
        ...
        if (at && type === LightType.Point)
            l.position.copy(at).setY(at.y + 20);
        this.lights.push(l);
        this.touch();
        return l;
    }
