...
// ---- serialize ----
export function serializeScene(scene, hooks) {
...
export function deserializeScene(scene, json) {
    const o = JSON.parse(json);
    if (!o || typeof o.version !== "number")
        throw new Error("Not a CSBryce scene file.");
    ...
    clear(scene.blasMatCount);
    clear(scene.blasFile);
    clear(scene.meshMaterials);
    clear(scene.meshMaterialNames);
    clear(scene.meshTexLayers);
    clear(scene.meshNormalLayers);
    clear(scene.primImageLayers);
    ...
    for (const sp of o.prims ?? []) {
        ...
        p.c = sp.c;
        p.color = sp.color;
        p.reflectivity = sp.reflectivity;
        p.pattern = sp.pattern;
        p.colorB = sp.colorB;
        p.patternScale = sp.patternScale;
        p.bump = sp.bump;
        p.bumpScale = sp.bumpScale;
        p.imageLayer = sp.imageLayer;
        p.cornerRadius = sp.cornerRadius ?? 0;
        p.subtractive = !!sp.subtractive;
        p.group = sp.group ?? 0;
        scene.prims.push(p);
    }
    for (const sl of o.lights ?? []) {
        ...
        l.softness = sl.softness;
        l.range = sl.range;
        l.bodyRadius = sl.bodyRadius ?? 0;
        l.inSky = sl.inSky ?? false;
        l.ringInner = sl.ringInner ?? 1.5;
        l.ringOuter = sl.ringOuter ?? 2.4;
        l.ringOpacity = sl.ringOpacity ?? 0;
    ...
    for (const s of mesh.texLayers ?? [])
        scene.meshTexLayers.push(b64ToU8(s));
    for (const s of mesh.normalLayers ?? [])
        scene.meshNormalLayers.push(b64ToU8(s));
    for (const si of mesh.instances ?? []) {
        ...
        m.scale = si.scale;
        m.subtractive = !!si.subtractive;
        m.group = si.group ?? 0;
        if (si.landform)
            m.landform = si.landform; // restore procedural-landform recipe + seed
        scene.instances.push(m);
    }
    const pi = o.primImages ?? { size: 256, layers: [] };
    scene.primImageSize = pi.size ?? 256;
    for (const s of pi.layers ?? [])
        scene.primImageLayers.push(b64ToU8(s));
...
function texFromBytes(bytes, srgb) {
...
export function previewFromBlas(scene, blasIndex) {
    ...
    const groups = [];
    let cur = -1;
    const decode = (packed, d) => {
        const p = packed | 0;
        col[d] = (p & 255) / 255;
        col[d + 1] = ((p >> 8) & 255) / 255;
        col[d + 2] = ((p >> 16) & 255) / 255;
    };
    for (let ti = 0; ti < n; ti++) {
        const o = order[ti] * TRI_STRIDE;
        const mid = Math.max(0, (t[o + 30] | 0) - base);
        if (mid !== cur) {
            groups.push({ start: ti * 3, count: 0, mat: mid });
            cur = mid;
        }
        ...
        for (let v = 0; v < 3; v++) {
            ...
            pos[d3 + 2] = t[o + v * 4 + 2];
            nor[d3] = t[o + 12 + v * 4];
            nor[d3 + 1] = t[o + 12 + v * 4 + 1];
            nor[d3 + 2] = t[o + 12 + v * 4 + 2];
            uv[d2] = t[o + 24 + v * 2];
            uv[d2 + 1] = t[o + 24 + v * 2 + 1];
            decode(cw[v], d3);
        }
    }
    ...
    for (const g of groups)
        geom.addGroup(g.start, g.count, g.mat);
    const mats = [];
    for (let k = 0; k < count; k++) {
        ...
        if (d.tex)
            m.map = texFromBytes(d.tex, true);
        if (d.normal)
            m.normalMap = texFromBytes(d.normal, false);
        mats.push(m);
    }
