...
import { Scene, PrimType, LightType, type Primitive } from "./scene";
import { worldTerrainHeight } from "../gen/terrainField";

/** A framing hint: where the camera should sit for a scene's hero shot. Used for
 *  the gallery thumbnail and as the initial view when the scene is loaded. */
...
function ground(s: Scene, p: Primitive, x: number, z: number): void {
...
export const SCENE_PRESETS: ScenePreset[] = [
  ...
  },
  {
    ...
    build: () => {
      ...
      r1.a = 4; r1.color = [0.5, 0.34, 0.22]; r1.reflectivity = 0.02; r1.name = "Boulder";
      ground(s, r1, 20, 12);
      const r2 = s.addPrim(PrimType.Box);
      r2.a = 3; r2.b = 4; r2.c = 3; r2.rotation.y = 0.7; r2.color = [0.55, 0.4, 0.28]; r2.reflectivity = 0.02; r2.name = "Slab";
      ground(s, r2, -16, 22);
      s.touch();
      return s;
    },
  ...
  },
  {
    ...
    build: () => {
      ...
      a.a = 6; a.color = [0.95, 0.96, 0.98]; a.reflectivity = 0.92; a.name = "Chrome Sphere";
      ground(s, a, 12, 8);
      const b = s.addPrim(PrimType.Torus);
      b.a = 5; b.b = 1.8; b.rotation.x = 0.6; b.color = [0.8, 0.3, 0.25]; b.reflectivity = 0.4; b.name = "Ring";
      ground(s, b, -12, 14);
      const c = s.addPrim(PrimType.Octahedron);
      c.a = 5; c.color = [0.3, 0.7, 0.85]; c.reflectivity = 0.6; c.name = "Gem";
      ground(s, c, 0, -14);
      s.touch();
      return s;
    },
