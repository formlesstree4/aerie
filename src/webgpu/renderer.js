...
export class Renderer {
    ...
    particleBuffer;
    particleCount = 0;
    shutter = 0; // motion-blur shutter (seconds); set per-frame during video export
    partBound = new Float32Array(4); // cloud bounding sphere: cx,cy,cz,radius
    exposure = 1.05;
    warmth = 0.5;
    aperture = 0;
    focusDistance = 60;
    ...
    uploadParticles(data, count, shutter = 0, bound) {
        this.particleCount = Math.min(count, MAX_PARTICLES);
        this.shutter = shutter;
        if (bound)
            this.partBound.set(bound.subarray(0, 4));
        else
            this.partBound.fill(0);
    ...
    packEmitterLight(out, offset, e) {
        ...
        for (let k = 11; k < 20; k++)
            out[offset + k] = 0; // not-in-sky flag + ring fields
    }
    ...
    writeUniforms(cam) {
        ...
        d[40] = this.particleCount;
        d[41] = this.shutter;
        d[42] = 0;
        d[43] = 0;
        // row 11: particle bounding sphere (cx, cy, cz, radius)
        d[44] = this.partBound[0];
        d[45] = this.partBound[1];
        d[46] = this.partBound[2];
        d[47] = this.partBound[3];
