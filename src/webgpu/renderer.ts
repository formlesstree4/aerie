...
export class Renderer {
  ...
  private meshSampler: GPUSampler;
  private instCount = 0;
  private triBase: number[] = []; // per-BLAS triangle offset into the global pool
  private nodeBase: number[] = []; // per-BLAS node offset
  ...
  private presentBind!: GPUBindGroup;
  ...
  private readonly sunDir = new Vector3(0.5, 0.55, 0.3).normalize();
  private readonly sunColorV = new Vector3(1, 0.96, 0.9);
  private starsFlag = 0;
  private sunFlag = 1; // 1 when a directional light (sun) exists, else 0
  private sunBright = 3; // sun-disk brightness, driven by the sun light's intensity
  private primCount = 0;
  ...
  private makeTexture(size: number, layers: number): GPUTexture {
  ...
  private computeEntries(accum: GPUBuffer): GPUBindGroupEntry[] {
    return [
      { binding: 0, resource: { buffer: this.uniformBuffer } },
  ...
  async renderToPixels(cam: OrbitCamera, w: number, h: number, samples: number): Promise<Uint8ClampedArray<ArrayBuffer>> {
    ...
    for (let y = 0; y < h; y++) img.set(src.subarray(y * bytesPerRow, y * bytesPerRow + w * 4), y * w * 4);
    readBuf.unmap();
  ...
  async renderToImage(cam: OrbitCamera, w: number, h: number, samples: number): Promise<Blob> {
  ...
  uploadPrimTextures(scene: Scene): void {
  ...
  uploadWorld(scene: Scene): void {
    const w = scene.world;
    const d = this.worldData;
    const layers = w.cloudLayers.slice(0, MAX_CLOUD_LAYERS);
    d[0] = w.terrainAmp; d[1] = w.terrainFreq; d[2] = w.terrainRidge; d[3] = w.terrainOffset;
    d[4] = w.terrainEnabled ? 1 : 0; d[5] = w.waterEnabled ? 1 : 0;
    d[6] = w.cloudsEnabled && layers.length > 0 ? 1 : 0; d[7] = layers.length;
    d[8] = w.zenith[0]; d[9] = w.zenith[1]; d[10] = w.zenith[2];
    d[11] = 6000 + (300 - 6000) * w.sunSize; // sun disk exponent (bigger size → smaller exp)
    d[12] = w.horizon[0]; d[13] = w.horizon[1]; d[14] = w.horizon[2]; d[15] = 0;
    d[16] = w.hazeDensity; d[17] = w.terrainOctaves; d[18] = w.terrainWarp; d[19] = w.terrainSeed;
    d[20] = w.waterColor[0]; d[21] = w.waterColor[1]; d[22] = w.waterColor[2]; d[23] = w.waterReflectivity;
    // star0: density, brightness, size, twinkle
    d[24] = w.starDensity; d[25] = w.starBrightness; d[26] = w.starSize; d[27] = w.starTwinkle;
    // star1: color.rgb, variation
    d[28] = w.starColor[0]; d[29] = w.starColor[1]; d[30] = w.starColor[2]; d[31] = w.starVariation;
    // star2: clustering, seed
    d[32] = w.starClustering; d[33] = w.starSeed; d[34] = 0; d[35] = 0;
    // terrain palette: low / rock / high
    d[36] = w.terrainLow[0]; d[37] = w.terrainLow[1]; d[38] = w.terrainLow[2]; d[39] = 0;
    d[40] = w.terrainRock[0]; d[41] = w.terrainRock[1]; d[42] = w.terrainRock[2]; d[43] = 0;
    d[44] = w.terrainHigh[0]; d[45] = w.terrainHigh[1]; d[46] = w.terrainHigh[2]; d[47] = 0;
    // recipe block: rec0 (layer bases/fractals), rec1 (layer-1 + terrace), rec2 (terrace/warp/palette thresholds)
    d[48] = w.terrainBasis; d[49] = w.terrainFractal; d[50] = w.terrainBasis2; d[51] = w.terrainFractal2;
    d[52] = w.terrainFreq2; d[53] = w.terrainOctaves2; d[54] = w.terrainWeight2; d[55] = w.terrainTerraceSteps;
    d[56] = w.terrainTerraceSharp; d[57] = w.terrainWarpFreq; d[58] = w.terrainSnowLine; d[59] = w.terrainSlopeRock;
    ...
    for (let i = 0; i < MAX_CLOUD_LAYERS; i++) {
      ...
      const L = layers[i];
      if (L) {
        d[o] = L.type; d[o + 1] = L.coverage; d[o + 2] = L.density; d[o + 3] = L.scale;
        d[o + 4] = L.altitude; d[o + 5] = L.seed; d[o + 6] = L.thickness; d[o + 7] = L.darkness;
        d[o + 8] = L.color[0]; d[o + 9] = L.color[1]; d[o + 10] = L.color[2]; d[o + 11] = 0;
      } else {
        for (let k = 0; k < 12; k++) d[o + k] = 0;
        d[o + 3] = 1;
      }
    }
  ...
  private uploadTexArray(
    layers: Uint8Array<ArrayBuffer>[],
    size: number,
    old: GPUTexture,
  ): GPUTexture {
    old.destroy();
    if (layers.length === 0) return this.makeTexture(1, 1);
  ...
  uploadMeshPools(scene: Scene): void {
    const storage = GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_DST;
  ...
  uploadMeshMaterials(scene: Scene): void {
    const matData = new Float32Array(Math.max(12, scene.meshMaterials.length));
    matData.set(scene.meshMaterials);
  ...
  private writeUniforms(cam: OrbitCamera): void {
    ...
    d[4] = cam.forward.x; d[5] = cam.forward.y; d[6] = cam.forward.z; d[7] = this.width / this.height;
    ...
    d[20] = this.width; d[21] = this.height; d[22] = this.frame + 1; d[23] = this.sunBright;
    // row 6: primCount, lightCount, instCount, sunFlag
    d[24] = this.primCount; d[25] = this.lightCount; d[26] = this.instCount; d[27] = this.sunFlag;
