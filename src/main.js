...
async function main() {
    ...
    function applySceneState(state, label) {
    ...
    function applyCameraHint(c, h) {
        const f = h ?? { target: [0, 6, 0], distance: 80, yaw: -0.6, pitch: 0.24 };
    ...
    async function renderThumbnail(hint) {
    ...
    function persistCustomScenes(list) {
        try {
    ...
    function exportCustomScene(entry) {
    ...
    async function importToGallery(file) {
        try {
    ...
    async function loadCustomScene(entry) {
    ...
    async function loadSceneFromText(text, label) {
        try {
    ...
    function restoreAtmo(a) {
        const w = scene.world;
        w.timeOfDay = a.timeOfDay;
        w.exposure = a.exposure;
        w.hazeDensity = a.haze;
        w.zenith = [...a.zenith];
        w.horizon = [...a.horizon];
        w.starsEnabled = a.stars;
        const sun = scene.lights.find((l) => l.type === LightType.Directional);
    ...
    const cutscene = {
        ...
        keyAtmo: (i) => ({ timeOfDay: cutsceneKeys[i].timeOfDay, exposure: cutsceneKeys[i].exposure, haze: cutsceneKeys[i].haze }),
        // Edit an atmosphere field on a keyframe and preview it live at the playhead.
        setKeyAtmo: (i, field, v) => {
