...
const fade = (t: number) => t * t * t * (t * (t * 6 - 15) + 10); // quintic
...
export function gradientNoise(x: number, z: number, seed: number): number {
  const ix = Math.floor(x), iz = Math.floor(z);
  const fx = x - ix, fz = z - iz;
  const dot = (cx: number, cz: number, dx: number, dz: number) => {
...
export function worleyNoise(x: number, z: number, seed: number): number {
  const ix = Math.floor(x), iz = Math.floor(z);
  let minD = 1e9;
...
export type NoiseBasis = "value" | "gradient" | "worley";
export function sampleBasis(basis: NoiseBasis, x: number, z: number, seed: number): number {
  if (basis === "gradient") return gradientNoise(x, z, seed);
  if (basis === "worley") return worleyNoise(x, z, seed);
...
export function fractal(
  basis: NoiseBasis, mode: FractalMode,
  ...
  for (let i = 0; i < octaves; i++) {
    ...
    if (mode === "ridged") n = 1 - Math.abs(n * 2 - 1);      // sharp crests
    else if (mode === "billow") n = Math.abs(n * 2 - 1);     // rounded lobes
    sum += amp * n;
    norm += amp;
    f *= lacunarity;
    amp *= gain;
...
export function terrace(h: number, steps: number, sharpness: number): number {
  if (steps <= 0) return h;
...
export type MaskKind = "none" | "peak" | "plateau";
/** Radial shaping mask for placeable landforms. x,z ∈ [-1,1]; r = radius cutoff. */
export function radialMask(kind: MaskKind, x: number, z: number, radius: number): number {
