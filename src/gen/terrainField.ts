...
import type { World } from "../scene/scene";

const fract = (x: number) => x - Math.floor(x);
const mix = (a: number, b: number, t: number) => a + (b - a) * t;
const clampi = (x: number, lo: number, hi: number) => Math.min(hi, Math.max(lo, x));
...
function gradNoise2(px: number, py: number): number {
  const ix = Math.floor(px), iy = Math.floor(py);
  const fx = px - ix, fy = py - iy;
  const ux = fx * fx * fx * (fx * (fx * 6 - 15) + 10);
  const uy = fy * fy * fy * (fy * (fy * 6 - 15) + 10);
  const dotg = (cx: number, cy: number, dx: number, dy: number) => {
...
function worleyNoise2(px: number, py: number): number {
  const ix = Math.floor(px), iy = Math.floor(py);
  let minD = 1e9;
...
function basisSample(basis: number, px: number, py: number): number {
  if (basis === 1) return gradNoise2(px, py);
  if (basis === 2) return worleyNoise2(px, py);
  return noise2(px, py);
}
...
// --- fractal + terrace -------------------------------------------------------
function fractalMode(basis: number, mode: number, px: number, py: number, octaves: number): number {
  let x = px, y = py, amp = 0.5, sum = 0, norm = 0;
  for (let i = 0; i < octaves; i++) {
    let n = basisSample(basis, x, y);
    if (mode === 1) n = 1 - Math.abs(n * 2 - 1);
    else if (mode === 2) n = Math.abs(n * 2 - 1);
    sum += amp * n;
    norm += amp;
    x *= 2; y *= 2; amp *= 0.5;
  }
...
function terraceFn(h: number, steps: number, sharp: number): number {
  if (steps <= 0) return h;
...
export function worldTerrainHeight(x: number, z: number, w: World): number {
  let px = x, pz = z;
...
export function worldTerrainColor(
  x: number, y: number, z: number, ny: number, w: World,
  ...
  r = mix(r, high[0], t2); g = mix(g, high[1], t2); b = mix(b, high[2], t2);

  const t3 = smoothstep(0, 0.06, y); // beach/flats lift at the shoreline
  r = mix(low[0] * 1.15, r, t3); g = mix(low[1] * 1.15, g, t3); b = mix(low[2] * 1.15, b, t3);
